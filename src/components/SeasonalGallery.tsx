...
const dynamicBottom = useMemo(() => {
  ...
  <p className="text-sm md:text-lg max-w-2xl mb-6">{t(descKey)}</p>

  <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto justify-center items-center">
    <button className="bg-black text-white px-6 py-3 text-sm md:text-base font-semibold rounded-md w-full md:w-auto">
      {t("button.readMore")}
    </button>
    <button className="border border-white text-white px-6 py-3 text-sm md:text-base font-semibold rounded-md w-full md:w-auto">
      ...
      <div key={season.id} className="w-1/2 md:flex-1 text-center">
        <div
          className={`font-semibold tracking-wider text-sm md:text-lg ${
            i === currentSeasonIndex ? "text-orange-400" : "text-white"
          }`}
        >
